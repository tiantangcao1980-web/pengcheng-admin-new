...
export const fileApi = {
  ...
  upload(file: File, path?: string, groupId?: number | null): Promise<SysFile> {
    ...
    // 不设置 Content-Type，让 axios 自动为 FormData 添加 boundary，否则后端无法解析
    return request({
      url: '/sys/file/upload',
      method: 'post',
      data: formData
  ...
  updateTextContent(id: number, content: string): Promise<void> {
    return request({ url: `/sys/file/text/${id}`, method: 'put', data: { content } })
  },

  getOfficePreviewUrl(filePath: string): Promise<{ previewUrl: string; fileUrl: string }> {
    return request({ url: '/sys/file/preview-url', method: 'get', params: { filePath } })
  },
