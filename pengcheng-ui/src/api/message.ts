...
export interface ChatUser {
  ...
  isBlocked?: boolean
  /** 会话标签：focus 关注 / starred 星标 / muted 静音 */
  category?: string
}
...
export const chatApi = {
  ...
  /** 获取会话分类列表（如关注、静音等），可选按分类筛选 */
  getCategories(category?: string): Promise<{ chatType: string; targetId: number; category: string }[]> {
    return request({ url: '/sys/chat/categories', method: 'get', params: category ? { category } : {} })
  },
  /** 设置会话分类（关注 focus / 星标 starred / 静音 muted） */
  setCategory(chatType: 'private' | 'group', targetId: number, category: string): Promise<void> {
    return request({ url: '/sys/chat/category', method: 'post', data: { chatType, targetId, category } })
  },
  /** 删除会话分类（取消关注/静音/星标） */
  removeCategory(chatType: 'private' | 'group', targetId: number): Promise<void> {
    return request({ url: '/sys/chat/category', method: 'delete', params: { chatType, targetId } })
  }
}
