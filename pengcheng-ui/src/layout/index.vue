...
function getIconName(key: string): string {
  const iconMapping: Record<string, string> = {
    ...
    '/realty/stats': 'StatsChartOutline',
    '/ai/chat': 'SparklesOutline',
    '/ai/knowledge': 'LibraryOutline',
    '/ai/experiment': 'PulseOutline'
  }
...
// 将后台菜单数据转换为 Naive UI 菜单格式（与「系统管理-菜单管理」配置完全一致）
function convertMenus(menus: typeof userStore.menus): MenuOption[] {
  ...
  return menus
    ...
    .map(menu => {
      // 确保菜单路径是绝对路径（以 / 开头），目录与菜单均统一
      let menuPath = menu.path?.trim() || ''
      if (menuPath && !menuPath.startsWith('/')) {
        menuPath = '/' + menuPath
      }
      if (!menuPath && menu.type === 1) {
        menuPath = `/menu-${menu.id}` // 目录无 path 时用唯一 key，仅用于展开
      }
      if (!menuPath) {
        menuPath = `/menu-${menu.id}`
      }
...
// 菜单配置：仅使用后端返回的菜单（与「菜单管理」一致），仅固定前置「首页」
const menuOptions = computed<MenuOption[]>(() => {
  ...
  const backendMenus = convertMenus(userStore.menus)
...
// 从后端菜单树中根据当前路径解析面包屑（与菜单管理一致）
function findBreadcrumbFromMenus(
  menus: typeof userStore.menus,
  path: string,
  parents: Array<{ path: string; title: string }> = []
): Array<{ path: string; title: string }> | null {
  if (!menus || !Array.isArray(menus)) return null
  const normalized = (path || '').replace(/\/$/, '') || '/'
  for (const menu of menus) {
    const raw = (menu.path?.trim() && !menu.path.startsWith('/') ? '/' + menu.path : menu.path) || ''
    const p = raw.replace(/\/$/, '') || `/menu-${menu.id}`
    const current = [...parents, { path: p, title: menu.name }]
    if (p === normalized) return current
    if (menu.children?.length) {
      const found = findBreadcrumbFromMenus(menu.children, path, current)
...
// 面包屑：优先从后端菜单树解析，否则首页 + 当前路径
const breadcrumbs = computed(() => {
  if (route.path === '/dashboard') {
    return [{ path: '/dashboard', title: '首页' }]
  }
  const fromMenu = findBreadcrumbFromMenus(userStore.menus, route.path)
  if (fromMenu?.length) {
    return [{ path: '/dashboard', title: '首页' }, ...fromMenu]
  }
  const segs = route.path.split('/').filter(Boolean)
  const items: Array<{ path: string; title: string }> = [{ path: '/dashboard', title: '首页' }]
  let acc = ''
  for (const seg of segs) {
    acc += '/' + seg
    items.push({ path: acc, title: seg })
  }
  return items
})
...
// 菜单点击（目录占位 key 如 /menu-123 无对应路由，仅展开不跳转）
function handleMenuClick(key: string) {
  if (typeof key === 'string' && key.startsWith('/menu-')) return
  router.push(key)
}
