      ...
      uni.chooseImage({
	    ...
	    handleChooseFile() {
	      ...
	      uni.chooseMessageFile({
	        ...
	        type: 'file',
	        success: async (res) => {
	          const file = Array.isArray(res.tempFiles) && res.tempFiles.length > 0 ? res.tempFiles[0] : null
	          if (file && file.path) {
	            await this.sendFileMessage(file.path)
	          }
	        }
	      })
	      // #endif
	      // #ifndef MP-WEIXIN
	      uni.showToast({title: '当前端不支持文件发送', icon: 'none'})
	      // #endif
	    },
	    async sendFileMessage(filePath) {
	      uni.showLoading({title: '发送中...'})
	      try {
	        const r = await uploadFile(filePath)
	        const url = r.data?.url || r.data
	        await sendMessage({receiverId: this.targetId, content: url, msgType: 3})
	        this.messages.push({
	          id: Date.now(), senderId: this.myUserId, senderAvatar: this.myAvatar,
	          receiverId: this.targetId, content: url, msgType: 3, sendTime: new Date().toISOString()
	        })
	        this.$nextTick(() => this.scrollToBottom())
	      } catch (e) {
    ...
    handleChooseImage() {
      ...
      uni.chooseImage({
	    ...
	    getFileName(fileUrl) {
	      ...
	      try {
	        const pure = String(fileUrl).split('?')[0]
	        const name = pure.substring(pure.lastIndexOf('/') + 1)
	      ...
	      uni.downloadFile({
	        ...
	        success: (res) => {
	          ...
	          uni.openDocument({
	            ...
	            showMenu: true,
	            fail: () => uni.showToast({ title: '当前文件暂不支持预览', icon: 'none' })
	          })
	        },
