	...
	<view class="page">
		<scroll-view scroll-y class="form-scroll">
			<view class="form-section">
				...
				<view class="form-item">
					<text class="form-label">垫佣金额 <text class="required">*</text></text>
					<input class="form-input" v-model="form.amount" type="digit" placeholder="请输入垫佣金额" />
				</view>
				<view class="form-item">
					<text class="form-label">垫佣原因</text>
					<textarea class="form-textarea" v-model="form.reason" placeholder="请输入垫佣原因" maxlength="200"></textarea>
				</view>
			</view>
		</scroll-view>
		<view class="btn-wrap">
...
<script>
	import { applyAdvance, getCustomerDeals } from '../../utils/api.js'

	export default {
		data() {
			return {
				form: { dealId: '', amount: '', reason: '' },
				dealList: [],
				selectedDealLabel: '',
				submitting: false
			}
		},
		onLoad() { this.loadDeals() },
		methods: {
			async loadDeals() {
				try {
			...
			async handleSubmit() {
				...
				if (!this.form.amount) return uni.showToast({ title: '请输入垫佣金额', icon: 'none' })
