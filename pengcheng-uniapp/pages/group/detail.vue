			...
			async loadGroupInfo() {
				...
				handleMemberTap(member) {
					...
					if (isOwner && member.role !== 2) {
						itemList.push(member.role === 1 ? '取消管理员' : '设为管理员')
						itemList.push('移出群聊')
					} else if (isOwnerOrAdmin && member.role !== 2) {
						itemList.push('移出群聊')
					}
					...
					uni.showActionSheet({
						...
						success: async (res) => {
							if (res.tapIndex === 0) {
								uni.navigateTo({ url: `/pages/chat/index?targetId=${member.userId}&name=${encodeURIComponent(member.userNickname || member.nickname)}&avatar=${encodeURIComponent(member.avatar || '')}` })
								return
							}

							if (isOwner && member.role !== 2 && res.tapIndex === 1) {
								try {
									await setGroupAdmin(this.groupId, member.userId, member.role !== 1)
									uni.showToast({ title: member.role === 1 ? '已取消管理员' : '已设为管理员', icon: 'success' })
							...
							const removeTapIndex = isOwner && member.role !== 2 ? 2 : 1
							if (res.tapIndex === removeTapIndex) {
								uni.showModal({
									title: '移出群聊',
									content: `确定将 ${member.userNickname || member.nickname || '该成员'} 移出群聊吗？`,
									confirmColor: '#FA5151',
									success: async (confirmRes) => {
										if (!confirmRes.confirm) return
										try {
											await removeGroupMember(this.groupId, member.userId)
					...
					uni.navigateTo({
						...
						events: {
							selectedUsers: async (selectedUsers = []) => {
								...
								try {
									await addGroupMembers(this.groupId, userIds)
...
<style lang="scss" scoped>
	.page { min-height: 100vh; min-height: 100dvh; height: 100vh; height: 100dvh; background: #EDEDED; }
	.content { height: 100%; }
