...
public class GetuiPushService implements PushService {
    ...
    public GetuiPushService(String appKey, String masterSecret) {
        ...
        log.info("初始化个推推送服务, appKey={}", PushLogSanitizer.maskIdentifier(appKey));
    }
    ...
    private boolean failClosed(String targetType, String target, String title, String content, Map<String, String> extras) {
        if (appKey == null || appKey.isBlank() || masterSecret == null || masterSecret.isBlank()) {
            log.error("【个推】配置不完整，拒绝发送: targetType={}, target={}", targetType, target);
            return false;
        }
        log.error("【个推】未实现实际供应商调用，拒绝假成功: targetType={}, target={}, title={}, content={}, extras={}",
