package com.pengcheng.file.service.impl;
...
import com.pengcheng.file.mapper.SysFileMapper;
...
import com.pengcheng.system.storage.FileStorageFactory;
...
public class SysFileServiceImpl implements SysFileService {
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public void updateFileContent(Long id, byte[] content) {
        SysFile file = fileMapper.selectById(id);
        if (file == null) {
            throw new RuntimeException("文件不存在");
        }
        ...
        String fileName = lastSlash >= 0 && lastSlash < filePath.length() - 1 ? filePath.substring(lastSlash + 1) : filePath;
        storage.delete(filePath);
        storage.upload(new ByteArrayInputStream(content), path, fileName);
        file.setFileSize((long) content.length);
        fileMapper.updateById(file);
        log.info("文件内容已更新: id={}, size={}", id, content.length);
    }
