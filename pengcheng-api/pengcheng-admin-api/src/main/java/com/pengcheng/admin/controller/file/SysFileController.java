...
public class SysFileController {
    ...
    @PutMapping("/text/{id}")
    @SaCheckPermission("sys:file:upload")
    @Log(title = "编辑文本文件", businessType = BusinessType.UPDATE)
    public Result<Void> updateTextContent(@PathVariable Long id, @RequestBody UpdateTextRequest request) {
        SysFile sysFile = fileService.getById(id);
        if (sysFile == null) {
            return Result.fail("文件不存在");
        }
        ...
        // 文本在线编辑限制 5MB，避免误编辑超大文件
        if (bytes.length > 5 * 1024 * 1024) {
            return Result.fail("文件过大，无法在线编辑");
        }

        fileService.updateFileContent(id, bytes);
        return Result.ok();
    }
