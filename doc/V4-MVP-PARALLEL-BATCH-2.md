...
## 6. 红线修订（rev.2，2026-04-26 紧急）

**事故**：协调人 rev.1 把 `sql/` 目录当作 Flyway 真实迁移目录，分配 V10-V23 给五路 agent。**真实 Flyway 目录是 `pengcheng-starter/src/main/resources/db/migration/`，已用到 V42**（含 V41__org_invite.sql、V42__meeting_calendar_api.sql）。

**修订方案**：
- D3 已自行改用 V43-V47（线索/自定义字段/visit-media/tag/realty-ext），合规接收。
- **D1 调整**：实际 V48-V49（tenant + user_login_device）；并且 `pengcheng-core/pengcheng-system/invite/` **已由 codex p1-dev 落地**（OrgInviteService / OrgInviteController + V41 表 sys_org_invite），D1 应在其上**扩展四种渠道**（短信/链接/二维码/Excel 导入），不得重写。
- **D2 调整**：V50-V52（attendance_shift/correction/approval_flow）。
- **D4 调整**：V53-V54（ai_reminder_rule/ai_copilot_action_log）。
- **D5 调整**：V55-V56（push_channel_log/subscribe_msg_template）。
- 由于 SendMessage 接口不可用，无法对已派出的后台 agent 追加指示；**协调人在 D1/D2/D4/D5 完工后，统一在合并阶段对各自 worktree 的 V 文件做改名 + 内容重映射**。
- D5 任务清单中"OCR 子包"原写在 `pengcheng-system/`，与 D1 同模块；改为允许 D5 在新建 `pengcheng-core/pengcheng-system/ocr-card/` 子包工作，D1 严禁碰 `ocr-card/`。
...
| 2026-04-26 | D3（CRM）完工，揭露 Flyway 真实目录 V42 已占。rev.2 修订红线，协调人接管合并阶段 V 编号统一。 |
